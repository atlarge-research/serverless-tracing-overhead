...
def aggregate_times_data(df):
    aggregation = {
        'benchmark_time': ['mean', 'std', lambda x: x.quantile(0.99)],
        'client_time': ['mean', 'std', lambda x: x.quantile(0.99)]
    }
    agg_df = df.groupby(['benchmark_name', 'configuration']).agg(aggregation).reset_index()
    agg_df.columns = [
        'benchmark_name', 'configuration',
        'benchmark_time_mean', 'benchmark_time_std', 'benchmark_time_p99',
        'client_time_mean', 'client_time_std', 'client_time_p99'
    ]

    # Round the results to two decimals
    agg_df = agg_df.round({
        'benchmark_time_mean': 2, 'benchmark_time_std': 2, 'benchmark_time_p99': 2,
        'client_time_mean': 2, 'client_time_std': 2, 'client_time_p99': 2
    })
