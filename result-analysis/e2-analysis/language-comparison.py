    ...
    for language in df['language'].unique():
        for benchmark_name in df['benchmark_name'].unique():
            ...
            if not non_instrumented.empty and not instrumented.empty:
                ...
                # Add absolute mean values for comparison
                avg_mean = (non_instrumented_mean + instrumented_mean) / 2
...
def compare_tracing_overhead(python_dir, nodejs_dir, time_type='benchmark'):
    ...
    for benchmark_name in comparison_df['benchmark_name'].unique():
        ...
        if not python_data.empty and not nodejs_data.empty:
            ...
            # Calculate mean values for Python and Node.js
            python_mean = python_data['average_mean'].values[0]
            nodejs_mean = nodejs_data['average_mean'].values[0]
        overhead_diff_data.append({
            'benchmark_name': benchmark_name,
            'python_overhead': python_overhead,
            'python_non_inst_mean': round(python_data['non_instrumented_mean'].values[0], 2),
            'python_instr_mean': round(python_data['instrumented_mean'].values[0], 2),
            # 'python_mean': round(python_mean, 2),
            'nodejs_overhead': nodejs_overhead,
            'nodejs_non_inst_mean': round(nodejs_data['non_instrumented_mean'].values[0], 2),
            'nodejs_instr_mean': round(nodejs_data['instrumented_mean'].values[0], 2),
            # 'nodejs_mean': round(nodejs_mean, 2),
...
print("\nBenchmark")
benchmark_overhead_diff_df.to_csv('language_comparison.csv', sep='\t')
