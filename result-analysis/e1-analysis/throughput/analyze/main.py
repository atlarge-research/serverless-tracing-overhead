...
def plot_language_comparison_boxplot(file_path, configuration):
    ...
    filtered_data = data[(data['language'].isin(['python', 'java', 'go', 'nodejs'])) & (data['configuration'] == configuration)]
    ...
    nodejs_data = filtered_data[filtered_data['language'] == 'nodejs']['targetRPS'].dropna().tolist()

    # Organize the data into a list for the boxplot
    data_to_plot = [python_data, java_data, go_data, nodejs_data]

    # Set up the matplotlib figure
    fig, ax = plt.subplots(figsize=(10, 6))

    # Create the boxplot
    ax.boxplot(data_to_plot, labels=['Python', 'Java', 'Go', 'Node.js'])
...
def plot_aggregated_RPS_across_variations(file_path):
    ...
    # Add annotations for min, max, median, mean, and upper whisker values
    for i, config in enumerate(order):
        ...
        ax.text(i + 1.15, min_val - 0, f'Min:\n{min_val:.2f}', va='center', fontsize=annotation_fontsize, color='black',
                bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))

        # Add annotation for max
        ax.text(i + 0.6, max_val + (200 if config == 'elastic' else 0), f'Max:\n{max_val:.2f}', va='center',
                fontsize=annotation_fontsize, color='black',
                bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))

        # Add annotation for median
        ax.text(i + 0.55, median_val + 100, f'Median:\n{median_val:.2f}', va='center', fontsize=annotation_fontsize,
                color='black',
                bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))

        # Add annotation for mean
        ax.text(i + 1.15, mean_val + 150, f'Mean:\n{mean_val:.2f}', va='center', fontsize=annotation_fontsize,
                color='black',
                bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))

        # Upper whisker
        # ax.text(i + 1.10, upper_whisker_val - 100, f'Upper Whisker:\n{upper_whisker_val:.2f}', va='center', fontsize=annotation_fontsize,
        #         color='black',
        #         bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))

        # Plot the mean as a red point
        ax.plot(i + 1, mean_val, 'ko', markersize=8, label='Mean' if i == 0 else "")
        # ax.plot(i + 1, min_val, 'ro', markersize=8, label='Min' if i == 0 else "")
...
def calculate_differences(file_path):
    ...
    pivot_table['otel_vs_standard'] = round(((pivot_table['otel'] - pivot_table['standard']) / pivot_table['standard']) * 100, 2)
    pivot_table['elastic_vs_standard'] = round(((pivot_table['elastic'] - pivot_table['standard']) / pivot_table[
        'standard']) * 100, 2)
    pivot_table['otel_vs_elastic'] = round(((pivot_table['otel'] - pivot_table['elastic']) / pivot_table['elastic']) * 100, 2)
    ...
    overall['otel_vs_standard'] = round(((overall['otel'] - overall['standard']) / overall['standard']) * 100, 2)
    overall['elastic_vs_standard'] = round(((overall['elastic'] - overall['standard']) / overall['standard']) * 100, 2)
    overall['otel_vs_elastic'] = round(((overall['otel'] - overall['elastic']) / overall['elastic']) * 100, 2)
